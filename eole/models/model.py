...
class DecoderModel(BaseModel):
    ...
    @torch.no_grad()
    def draft_mtp_tokens(self, h_last, seed_token, pos_id):
        """Greedily draft extra candidate tokens for self-speculative decoding
        using the MTP auxiliary heads. Inference-only helper: it never runs
        during training and has no effect on the auxiliary-loss computation
        performed in :meth:`forward`.

        Each MTP head reuses the *same* hidden state ``h_last`` (the main
        decoder's last-position output, pre-generator) combined with the
        embedding of the previous token in the chain — the just-generated
        token for the first head, the previous head's own draft for
        subsequent heads — mirroring the teacher-forced combination used
        during training (see :meth:`forward`): head ``k`` combines ``h_t``
        with the embedding of the token at position ``t + k`` to predict the
        token at position ``t + k + 1``.

        Args:
            h_last (Tensor): ``(batch, 1, hidden)`` hidden state that produced
                ``seed_token``, i.e. the main decoder output at position
                ``pos_id`` (pre-generator), used to predict the token at
                ``pos_id + 1`` which is ``seed_token``.
            seed_token (Tensor): ``(batch, 1)`` token ids just generated by
                the main model at absolute position ``pos_id + 1``.
            pos_id (int): absolute position of ``h_last`` in the sequence.

        Returns:
            list[Tensor]: one ``(batch, 1)`` greedily-drafted token id tensor
            per MTP head (empty list when MTP is disabled), for absolute
            positions ``pos_id + 2 .. pos_id + 1 + len(mtp_heads)``.
        """
        draft_tokens = []
        ...
        # Single cached hidden state, single candidate token: trivial 1x1
